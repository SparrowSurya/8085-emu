...
fn main() {
    ...
    println!("Stack Pointer (SP): 0x{:04X} (Expected: 0x1122)", cpu.regs.sp.0);
    ...
    println!("Stack memory at 0x0FFE: 0x{:02X} (Expected: 0xBB)", ram.read(Addr(0x0FFE)));
    println!("Stack memory at 0x0FFF: 0x{:02X} (Expected: 0xAA)", ram.read(Addr(0x0FFF)));
