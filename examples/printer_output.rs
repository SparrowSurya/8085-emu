...
fn main() {
    ...
    println!("Printed character buffer: {:?}", printed_chars.lock().unwrap());
    println!("Printer device history: {}", term.history);

    assert_eq!(*printed_chars.lock().unwrap(), vec!['A', 'B', 'C']);
}
