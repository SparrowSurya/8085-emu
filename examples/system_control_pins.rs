...
fn main() {
    ...
    println!(
        "Program Counter (PC): 0x{:04X} (Expected: 0x0000)",
        machine.cpu.regs.pc.0
    );
    println!(
        "Interrupts Enabled (inte): {} (Expected: false)",
        machine.cpu.inte
    );
    println!(
        "RESET_OUT Pin on bus: {} (Expected: true)",
        machine.bus.lines.reset_out
    );
    ...
    machine.tick();
    println!(
        "RESET_OUT Pin after releasing RESET_IN: {} (Expected: false)",
        machine.bus.lines.reset_out
    );
    assert!(!machine.bus.lines.reset_out);
    ...
    println!(
        "Did PC advance when READY=false? {} (Expected: true - PC does not advance)",
        machine.cpu.regs.pc == current_pc
    );
    println!(
        "CPU Cycle State: {:?}, T-state: {}",
        machine.cpu.cycle, machine.cpu.t_state
    );
    ...
    machine.tick();
    println!(
        "Did PC advance after asserting READY=true? {} (Expected: true)",
        machine.cpu.regs.pc != current_pc
    );
